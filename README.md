...
This repository provides two Hash Table implementations for use with STM32 microcontrollers: a Linear Probing Hash Table (LPHT) and a Cache Line Hash Table. Examples for both on-board memory, and external memory (MRAM) are included. We provide basic benchmarks to put/get values from the Hash Tables, which showcase the code necessary for basic setup and operation. The benchmarks use pre-generated key datasets of varying sizes (between 2 and 256 bytes) for evaluating performance. 
...
The `KEYS_FILE` indicates the file with the dataset to be used; `NUM_RECORDS_TO_INSERT` states the number of keys to insert, and should not be greater than the size of the dataset; and `FIELD_SIZE` indicates the size of the keys in bytes. The number of keys in each dataset is dependent on the key size: 
...
The displayed values are valid for 32 byte-sized keys. `NUM_BYTES_CAPACITY` sets the total amount of space available to the LPHT. `USABLE_CAPACITY` sets how much of that space is available for storing key/value pairs. The difference between the two, $NUM\_BYTES\_CAPACITY - USABLE\_CAPACITY$, is the amount of reserved space for slot occupation information. Currently, the value of USABLE_CAPACITY has to be set by hand, and can be calculated as:
...
Running LPHT on an external memory device is simple. You only have to change the LPHT pointers to match the location of the external device, and adjust the memory capacity of the table to be an accordance with the limitations of the device. The project includes code to run the Hash Table on an external MRAM device set at address `0xc0000000`. 
